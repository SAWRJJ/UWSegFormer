...
def RecoverSUTR(sceneRadiance):
    ...
    # clahe = cv2.createCLAHE(clipLimit=2, tileGridSize=(2, 2))
    for i in range(3):
        sceneRadiance[:, :, i] = (np.subtract(sceneRadiance[:, :, i],
                                              sceneRadiance[:, :, i] / float(np.sum(sceneRadiance[:, :, i]))) * float(
            np.min(sceneRadiance[:, :, i]))) + sceneRadiance[:, :, i]
    sceneRadiance = np.clip(sceneRadiance * 255, 0, 255)
    sceneRadiance = np.uint8(sceneRadiance)
    return sceneRadiance
...
def RecoverCTR(sceneRadiance):
    ...
    # clahe = cv2.createCLAHE(clipLimit=2, tileGridSize=(2, 2))
    for i in range(3):
        sceneRadiance[:, :, i] = (np.subtract(sceneRadiance[:, :, i], sceneRadiance[:, :, i] / 0.5) * float(
            np.min(sceneRadiance[:, :, i]))) + sceneRadiance[:, :, i]
    sceneRadiance = np.clip(sceneRadiance * 255, 0, 255)
    sceneRadiance = np.uint8(sceneRadiance)
    return sceneRadiance
